...
const DECISION_LABELS = {
  continue:         'Continue Interviewing',
  narrow_icp:       'Narrow ICP',
  pivot:            'Pivot',
  move_to_solution: 'Move to Solution Interview',
};
...
function buildReportText(report) {
  ...
  lines.push('Nitor8 — Overall Report');
  ...
  if (r.respondent_count != null) lines.push(`📊 ${r.respondent_count} Respondents`);

  if (r.overall_verdict) {
    const { status, evidence_level, reason } = r.overall_verdict;
    const parts = [status, evidence_level].filter(Boolean).join(' / ');
    lines.push(`🎯 Verdict: ${parts}`);
    if (reason) lines.push(reason);
  }

  if (r.pattern_summary) {
    lines.push(''); lines.push('Pattern Summary:'); lines.push(r.pattern_summary);
  }

  if (r.recurring_pains?.length) {
    lines.push(''); lines.push('Recurring Pains:');
    r.recurring_pains.forEach(p => {
      lines.push('');
      lines.push(p.frequency ? `${p.title} (${p.frequency})` : (p.title ?? ''));
      if (p.description) lines.push(p.description);
      if (p.representative_quote) lines.push(`"${p.representative_quote}"`);
    });
  }

  if (r.common_workarounds?.length) {
    lines.push(''); lines.push('Common Workarounds:');
    r.common_workarounds.forEach(w => {
      lines.push('');
      lines.push(w.frequency ? `${w.method} (${w.frequency})` : (w.method ?? ''));
      if (w.complaint) lines.push(`✕ ${w.complaint}`);
    });
  }

  const quotes = r.key_evidence_quotes?.filter(Boolean) ?? [];
  if (quotes.length) {
    lines.push(''); lines.push('Key Evidence Quotes:');
    quotes.forEach(q => { lines.push(''); lines.push(`"${q}"`); });
  }

  const actions = r.next_actions?.filter(Boolean) ?? [];
  if (actions.length) {
    lines.push(''); lines.push('Next Actions:');
    actions.forEach(a => lines.push(`→ ${a}`));
  }

  const questions = r.open_questions?.filter(Boolean) ?? [];
  if (questions.length) {
    lines.push(''); lines.push('Open Questions:');
    questions.forEach(q => { lines.push(''); lines.push(q); });
  }

  if (r.decision_recommendation) {
    lines.push('');
    lines.push(`Decision: ${DECISION_LABELS[r.decision_recommendation] ?? r.decision_recommendation}`);
  }

  lines.push(''); lines.push('Generated by Nitor8 — nitor8.vercel.app');
  return lines.join('\n');
}
